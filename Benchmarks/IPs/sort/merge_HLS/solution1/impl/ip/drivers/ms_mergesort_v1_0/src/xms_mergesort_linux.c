...
typedef struct {
    ...
    XMs_mergesort_uio_map maps[ MAX_UIO_MAPS ];
} XMs_mergesort_uio_info;

/***************** Variable Definitions **************************************/
static XMs_mergesort_uio_info uio_info;
...
static int uio_info_read_name(XMs_mergesort_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMs_mergesort_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMs_mergesort_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMs_mergesort_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMs_mergesort_Initialize(XMs_mergesort *InstancePtr, const char* InstanceName) {
	XMs_mergesort_uio_info *InfoPtr = &uio_info;
