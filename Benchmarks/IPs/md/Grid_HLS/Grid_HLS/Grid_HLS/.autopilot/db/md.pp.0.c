...
__attribute__((sdx_kernel("md", 0))) void md( int32_t n_points[4][4][4],
         dvector_t force[4][4][4][10],
         dvector_t position[4][4][4][10]
       );



struct bench_args_t {
  int32_t n_points[4][4][4];
  dvector_t force[4][4][4][10];
  dvector_t position[4][4][4][10];
};
# 2 "../CCode_grid/md.c" 2
...
__attribute__((sdx_kernel("md", 0))) void md( int n_points[4][4][4],
         dvector_t force[4][4][4][10],
         dvector_t position[4][4][4][10] )
{_ssdm_SpecArrayDimSize(n_points, 4);_ssdm_SpecArrayDimSize(force, 4);_ssdm_SpecArrayDimSize(position, 4);
#pragma HLS TOP name=md
# 9 "../CCode_grid/md.c"


#pragma HLS INTERFACE s_axilite port=n_points bundle=BUS_A
#pragma HLS INTERFACE s_axilite port=force bundle=BUS_A
#pragma HLS INTERFACE s_axilite port=position bundle=BUS_A
#pragma HLS INTERFACE s_axilite port=return bundle=BUS_A
 ...
 ivector_t b0, b1;
  ...
  loop_grid0_x: for( b0.x=0; b0.x<4; b0.x++ ) {
  loop_grid0_y: for( b0.y=0; b0.y<4; b0.y++ ) {
  loop_grid0_z: for( b0.z=0; b0.z<4; b0.z++ ) {

  loop_grid1_x: for( b1.x=( (0)>(b0.x-1) ? (0) : (b0.x-1) ); b1.x<( (4)<(b0.x+2) ? (4) : (b0.x+2) ); b1.x++ ) {
  loop_grid1_y: for( b1.y=( (0)>(b0.y-1) ? (0) : (b0.y-1) ); b1.y<( (4)<(b0.y+2) ? (4) : (b0.y+2) ); b1.y++ ) {
  loop_grid1_z: for( b1.z=( (0)>(b0.z-1) ? (0) : (b0.z-1) ); b1.z<( (4)<(b0.z+2) ? (4) : (b0.z+2) ); b1.z++ ) {
